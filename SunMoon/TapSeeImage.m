    ...
    if (clickViewController!=nil) {
        ...
        [UIView animateWithDuration:_duration animations:^{
            ...
            [UIView animateWithDuration:(_duration/2) animations:^{
                ...
                ((tapToImage*)clickViewController).showImage =imageView;
                
            } completion:^(BOOL finished) {
                goBackgroundView.alpha = 0.0;
                for (UIView* next = [self superview]; next; next = next.superview) {
                    UIResponder *nextResponder = [next nextResponder];
                    if ([nextResponder isKindOfClass:[HomeInsideViewController class]]) {
                        [((UIViewController*)nextResponder) presentViewController:clickViewController animated:NO completion:^{
                
                            //未起作用？
                            ((tapToImage*)clickViewController).showImage.image =imageView.image;
